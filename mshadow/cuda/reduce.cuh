...
inline __device__ void ReduceX(volatile DType  buf[], int tid) {
  ...
  if (tid < 32) {
    if (x_bits >= 6) {
      Reducer::Reduce(buf[tid] , buf[tid + 32]);
      __MSHADOW_EMUSYNC__;
    }
    if (x_bits >= 5) {
      Reducer::Reduce(buf[tid] , buf[tid + 16]);
      __MSHADOW_EMUSYNC__;
    }
    if (x_bits >= 4) {
      Reducer::Reduce(buf[tid] , buf[tid + 8]);
      __MSHADOW_EMUSYNC__;
    }
    if (x_bits >= 3) {
      Reducer::Reduce(buf[tid] , buf[tid + 4]);
      __MSHADOW_EMUSYNC__;
    }
    if (x_bits >= 2) {
      Reducer::Reduce(buf[tid] , buf[tid + 2]);
      __MSHADOW_EMUSYNC__;
    }
    if (x_bits >= 1) {
      Reducer::Reduce(buf[tid] , buf[tid + 1]);
      __MSHADOW_EMUSYNC__;
    }
  }
}
